"""
基于 Qwen3-0.6B GPTQ Int4 量化的本地关键词检测 (Function Call)
使用官方 gptqmodel (TritonV2QuantLinear) 加载真正的 GPTQ 权重进行推理。
8个本地关键词:
  耳机拍照, 手机拍照, 上一曲, 下一曲, 音量增大, 音量减小, 暂停播放, 继续播放
"""
import os
# 避免 gptqmodel 触发 torch.compile 相关的潜在问题
...
def main():
    ...
    parser.add_argument('--model_path', default='../Qwen3-0.6B-GPTQ-Int4')
    parser.add_argument('--data_path', default='./dataset/local_keywords_samples.json')
    parser.add_argument('--output_path', default='./keyword_eval_gptq.json')
    ...
    if args.device == 'auto':
        ...
        device = args.device
    print(f'设备: {device}')
    print(f'模型路径 (GPTQ Int4): {args.model_path}')
    print('加载 tokenizer 和 model (gptqmodel)...')

    t0 = time.time()
    tokenizer = AutoTokenizer.from_pretrained(args.model_path)
    model = GPTQModel.load(args.model_path, device=device)
    ...
    print(f'\n开始评估 GPTQ Int4 (每关键词{per_kw if per_kw else "全部"}条)...\n')

    eval_result = evaluate(model, tokenizer, samples, model.device, sample_per_keyword=per_kw)
    ...
    output = {
        ...
        'loader': 'gptqmodel(TritonV2QuantLinear)',
