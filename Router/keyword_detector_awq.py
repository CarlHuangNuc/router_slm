"""
基于 Qwen3-0.6B AWQ int4 量化的本地关键词检测 (Function Call)
8个本地关键词:
...
FUNCTIONS = [
    {
        "name": "earphone_photo",
        "description": "通过耳机按键触发拍照(下发到耳机端执行)",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "phone_photo",
        "description": "用手机相机拍照。注意:'帮我拍照'/'拍一下'等未指明设备的拍照请求默认为手机拍照",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "previous_track",
        "description": "切换到上一首歌曲/音频(媒体控制)",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "next_track",
        "description": "切换到下一首歌曲/音频(媒体控制),包括切歌/跳过当前歌曲",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "volume_up",
        "description": "调高音量/声音变大",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "volume_down",
        "description": "调低音量/声音变小",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "pause_playback",
        "description": "暂停当前播放(音乐/视频/音频)",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "resume_playback",
        "description": "继续/恢复播放,包括开始播放音乐",
        "parameters": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "unknown",
        "description": "用户意图与上述8个本地关键词都不匹配,需走云端处理",
        "parameters": {"type": "object", "properties": {}, "required": []}
    }
]
...
def main():
    ...
    parser.add_argument('--model_path', default='./Qwen3-0_6B_int4_awq')
    parser.add_argument('--data_path', default='./dataset/local_keywords_samples.json')
    parser.add_argument('--output_path', default='./keyword_eval_awq.json')
    ...
    if args.device == 'auto':
        ...
        device = args.device
    print(f'设备: {device}')
    print(f'模型路径 (AWQ int4): {args.model_path}')
    ...
    # 打印模型量化信息
    if hasattr(model, 'hf_quantizer') and hasattr(model.hf_quantizer, 'quantization_config'):
        print(f'量化配置: {model.hf_quantizer.quantization_config}')
    ...
    print(f'\n开始评估 AWQ int4 (每关键词{per_kw if per_kw else "全部"}条)...\n')
