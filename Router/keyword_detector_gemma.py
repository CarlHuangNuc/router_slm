...
# Also map common English variants the model might output
EN_VARIANT_TO_FN = {
    'earphone_photo': 'earphone_photo',
    'phone_photo': 'phone_photo',
    'take_photo': 'phone_photo',
    'camera': 'phone_photo',
    ...
    'start_playback': 'resume_playback',
    'unknown': 'unknown',
}
...
def parse_function_call(output_text: str) -> str:
    ...
    # Extract from JSON or code block
    ...
    # Check exact English function name
    if raw_fn in EN_VARIANT_TO_FN:
        return EN_VARIANT_TO_FN[raw_fn]

    # Check Chinese keyword → function name
    if raw_fn in KEYWORD_TO_FN:
        return KEYWORD_TO_FN[raw_fn]

    # Fuzzy match: check if any known keyword/variant is contained in the output
    for variant, fn in EN_VARIANT_TO_FN.items():
        if variant in text.lower():
            return fn
    for kw in KEYWORD_TO_FN:
        if kw in text:
            return KEYWORD_TO_FN[kw]
...
def main():
    ...
    parser.add_argument('--model_path', default='../gemma-3-270m-it')
    parser.add_argument('--data_path', default='./dataset/local_keywords_zh.json')
    parser.add_argument('--output_path', default='./keyword_eval_gemma270m.json')
