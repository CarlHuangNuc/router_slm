...
from train_utils.modeling_qwen3 import Qwen3ForCausalLM as Qwen3ForCausalLMQuant
...
def train() -> None:
    ...
    if hasattr(ptq_args, 'calibration_data') and ptq_args.calibration_data:
        log.info(f"Loading calibration dataset from: {ptq_args.calibration_data}")
        calibration_datasets = datasets.load_dataset(
            'json', 
            data_files=ptq_args.calibration_data, 
            split='train[:]'
        )
    else:
        log.info("Using default wikitext dataset for calibration")
        calibration_datasets = datasets.load_dataset(
            "Salesforce/wikitext", 
            "wikitext-2-raw-v1",
            split="train"
