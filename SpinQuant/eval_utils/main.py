...
def update_model(model):
    rm_keys = ['had_K', 'scale', 'maxq', 'zero', 'module.weight', 'module.bias']

    # Create a list of keys to remove
    keys_to_remove = [key for key in model.keys() if any(rm_key in key for rm_key in rm_keys)]
...
def ptq_model(args, model, model_args=None, tokenizer=None):
    ...
    # Rotate the weights
    if args.rotate:
        print("fuse_layer_norms, rotate_model")
    ...
    if args.w_bits < 16:
        print(f'args.w_bits: {args.w_bits}')
        save_dict = {}
        if args.load_qmodel_path:  # Load Quantized Rotated Model
            ...
            if hasattr(args, 'calibration_data') and args.calibration_data:
                print(f"Loading calibration dataset from: {args.calibration_data}")
                trainloader = data_utils.get_json_data(
                    nsamples=args.nsamples,
                    seed=args.seed,
                    model=model_args.input_model,
                    seqlen=4096,
                    eval_mode=False,
                    json_data_path=args.calibration_data)
            else:
                print("Using default wikitext dataset for calibration")
        ...
        if args.save_qmodel_path:
            ...
            updated_model = update_model(model.state_dict())
            torch.save(updated_model, args.save_qmodel_path)

    # Add Input Quantization
    if args.a_bits < 16 or args.v_bits < 16:
        print(f'args.a_bits: {args.a_bits}')
    ...
    if args.k_bits < 16:
        print(f'args.k_bits: {args.k_bits}')
        if args.k_pre_rope:
