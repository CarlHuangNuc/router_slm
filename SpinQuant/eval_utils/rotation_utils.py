...
def rotate_mlp_output(layer, R1):
    ...
    if mlp is not None:
        if hasattr(mlp, "down_proj"):
            W = mlp.down_proj
        elif hasattr(mlp, "output_linear"):
            W = mlp.output_linear
...
def rotate_mlp_output_no_had(layer, R1):
    # Rotate the MLP output weights and bias.
    mlp = None
    if hasattr(layer, "mlp"):
        mlp = layer.mlp
...
def rotate_model_no_had(model, args):
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        ...
        rotate_mlp_output_no_had(layers[idx], R1)
